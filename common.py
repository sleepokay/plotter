# copied from https://github.com/dbalan/plotter-scripts
...
def exec_hpgl(cmds, port="/dev/tty.usbserial-10", speed=9600):
    body = stitch(cmds)
    with serial.Serial(port, speed, timeout=None) as plt:
        for ins in body:
            ...
            plt.write(ins.encode('ascii'))
            # For every line sent, end with OA, which reports back current
            # position on the pen
            plt.write("OA;".encode('ascii'))
            c = ""
            data = ""
            while c != '\r':
                c = plt.read().decode('ascii')
                data += c
